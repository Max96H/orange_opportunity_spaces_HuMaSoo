...
# Groq model alias (e.g. "groq/compound" or another supported Groq model like "llama-3.3-70b-versatile")
MODEL_ID_1 = os.getenv("GROQ_MODEL_ID_1", "openai/gpt-oss-20b")
MODEL_ID_2 = os.getenv("GROQ_MODEL_ID_2", "openai/gpt-oss-120b")


# Initialize Groq client
client = Groq(api_key=GROQ_API_KEY)

DB_PATH = "./data/sus_cyb_fullBody_signals.db"
...
def safe_api_call(messages: list, model_id: Optional[str] = None, max_tokens: int = 2048, retries: int = 3) -> str:
    target_model = model_id or MODEL_ID_1
    ...
    for attempt in range(retries):
        try:
            ...
            if status == 429:
                ...
                logging.warning(f"Rate limit hit (HTTP 429). Sleeping for {delay}s before retry...")
                time.sleep(delay)
                continue

            if status is not None and status in NON_RETRYABLE_STATUS_CODES:
            ...
            logging.warning(f"API attempt {attempt + 1}/{retries} failed: {e}. Retrying in {delay:.0f}s...")
...
def extract_technologies(domain: str, raw_articles: List[dict]) -> dict:
    ...
    # FIX: Limit strictly to 25 articles for Step 1, strip newlines & white space
    articles_payload = []
    ...
    raw_json_str = safe_api_call(messages, MODEL_ID_1, max_tokens=2048)
...
def generate_opportunity_space(domain: str, step1_result: dict, filtered_articles: List[dict]) -> dict:
    ...
    #     f"Articles:\n{articles_formatted}"
    # )
    # 1. Compact article formatting (saves ~10-15 tokens per article)
    articles_formatted = "\n".join(
        f"[{a['id']}] {a['url']} | {a['content'][:200].strip()}..."
        for a in filtered_articles
    )

    # 2. Compact JSON formatting for step1_result
    ...
    raw_json_str = safe_api_call(messages, MODEL_ID_2, max_tokens=4096)
